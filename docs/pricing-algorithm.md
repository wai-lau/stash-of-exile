...
| Waystones | exchange; search to sell, or a sixteen | The tier's bulk book prices the commodity and the tooltip totals become a loot score. A stone worth running (70 up) is run, not sold, and is not searched; one under that with a single total at its market cliff looks good, is not, and is searched to sell. A sixteen is always searched — its book is bait. See stage 05, and [waystones.md](waystones.md) for what the market was measured to pay. |
...
the fills stand aside — the trade-site books are live, and price
instead until poe2scout catches up. Its snapshots stopped for a day on
2026-08-27, and a fresh fetch of a day-old snapshot is a day-old price.
...
| Waystone | *search to sell* | A waystone worth running is not searched, and neither is one under every market cliff: a stone's search caps at 10,000 matches — a commodity — and the exchange carries that commodity by tier (Waystone (Tier 15) held 6,957 online units), so a search only ever spent a call to learn what the book already said. The price is the tier's bulk book. What separates one stone from another is its tooltip — since 0.5 every mod carries a fixed loot line and the tooltip sums them into item rarity, pack size, monster rarity, monster effectiveness — scored as loot: `1.0 × monster rarity + 1.0 × pack size + 0.5 × effectiveness + 0.5 × item rarity`, effectiveness exact from the game's glossary (1% more quantity per 2%), pack size a point of drops per point since it multiplies every pack, the rest judgement, banded under 50 reroll, 50 run it, 70 juice it, 90 chase, and the band is worn as a colour — grey, blue, yellow, orange, bold from yellow up. Under it, the Distilled Emotion to instill: the score is linear, so it is the same every time — Paranoia, +15 monster rarity — and the line gives the score and band one of it buys. Scores are integers and the band is the integer's, or a 64.6 prints as 65 and bands as "run it". A corrupted stone takes none; an instilled one has had its turn. A `danger` line calls out the mods that kill the player, red, one per row — −max resistances, penetration, extra chaos, less recovery, the desecrated no-damage and Marked for Death — since the score says nothing about whether the map can be run. The merely risky tier (extra elemental, more monster damage, crit, speed, projectiles, curses, less cooldown recovery, flask charges, ground patches) is counted in a dim row, not shown: it is most of every stone. The search is for SELLING: a stone from 70 (juice it) is run by its owner, and a search would price what is not for sale; one under that whose single total sits at its market cliff — monster rarity 60, pack size 35, item rarity 80, effectiveness 50, drop chance 120, measured 2026-08-28 in [waystones.md](waystones.md) — looks good, is not, and is searched on tier and the totals that sell it as floors — the one total is what the market prices, and five floors at once matched nothing where item rarity 80 alone was 148 listings — the loot row saying which total sells it. A comparable at least as good on all five is what the book by tier cannot say; when nothing that good is listed, the fifteen's book is reported as the floor. A sixteen is searched whatever it scores and never priced off its book — one AFK account's 63 stones at 1 ex under a 99-ex search floor — so when nothing as good is listed its floor is one more search on the tier alone. |
