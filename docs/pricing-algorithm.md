...
| Waystones | exchange, never search | The tier's bulk book prices the commodity; the tooltip totals become a loot score. See stage 05. |
...
| Waystone | *no search* | A waystone is never searched: a stone's search caps at 10,000 matches — a commodity — and the exchange carries that commodity by tier (Waystone (Tier 15) held 6,957 online units), so a search only ever spent a call to learn what the book already said. The price is the tier's bulk book. What separates one stone from another is its tooltip — since 0.5 every mod carries a fixed loot line and the tooltip sums them into item rarity, pack size, monster rarity, monster effectiveness — scored as loot: `1.0 × monster rarity + 0.5 × effectiveness + 0.5 × item rarity + 0.4 × pack size`, effectiveness exact from the game's glossary (1% more quantity per 2%), the rest judgement, banded under 45 reroll, 45 run it, 65 juice it, 80 chase. Under it, the instillation path: the score is linear, so the best three Distilled Emotions are the best one three times — Paranoia, +15 monster rarity each — and what varies by stone is where the band flips, so the line shows ×1, ×2, ×3 with the band at each and the third can be skipped when it buys nothing. A corrupted stone takes none; an instilled one has had its turn. The map filters (tier and the five totals as floors) stay wired for the day a search is wanted. |
