...
| Endgame (waystones, ultimatums…) | always search | No index covers them at all. |
...
| Waystone tier and totals | `min` | A waystone is bought on the totals at the top of the item, never its mods. Since 0.5 every waystone mod carries a fixed loot line and the tooltip sums them into five totals — item rarity, pack size, monster rarity, monster effectiveness, waystone drop chance — so the comparable is a stone at least as good on all five. Like a defence total, these survive every rung of the ladder. (Before 0.5, measured live on securable T16s, only item rarity moved the price; monster rarity is now the loot stat and that measurement is void.) The report also scores the totals as loot — `1.0 × monster rarity + 0.5 × effectiveness + 0.5 × item rarity + 0.4 × pack size`, effectiveness exact from the game's glossary (1% more quantity per 2%), the rest judgement — and bands it: under 45 reroll, 45 run it, 65 juice it, 80 chase. |
